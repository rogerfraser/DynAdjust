...
//
// Description  : DynAdjust binary station file I/O operations
//============================================================================
...
[[nodiscard]] std::ifstream open_bst_for_read(const std::string &filename) {
  std::ifstream stream{filename, std::ios::binary};
  stream.exceptions(std::ios::badbit | std::ios::failbit);
  if (!stream.is_open()) {
    std::ostringstream os;
    os << "load_bst_file(): could not open file '" << filename
       << "' for reading.";
...
[[nodiscard]] std::ofstream open_bst_for_write(const std::string &filename) {
  std::ofstream stream{filename, std::ios::binary | std::ios::trunc};
  stream.exceptions(std::ios::badbit | std::ios::failbit);
  if (!stream.is_open()) {
    std::ostringstream os;
    os << "write_bst_file(): could not open file '" << filename
       << "' for writing.";
    throw std::runtime_error(os.str());
  }
  return stream; // move-return
...
dna_io_bst &dna_io_bst::operator=(const dna_io_bst &rhs) noexcept {
  if (this != &rhs)
...
std::uint64_t
dna_io_bst::create_stn_input_file_meta(vifm_t &vinput_file_meta,
  ...
  try {
    ...
    *input_file_meta = nullptr;
    ...
    std::ostringstream os;
    os << "create_stn_input_file_meta(): " << e.what();
    throw std::runtime_error(os.str());
  }
}

void dna_io_bst::load_bst_file_meta(const std::string &bst_filename,
                                    binary_file_meta_t &bst_meta) {
  try {
    auto bst_file = open_bst_for_read(bst_filename);
    ...
    std::ostringstream os;
    os << "load_bst_file(): " << e.what();
    throw std::runtime_error(os.str());
  }
}

std::uint64_t dna_io_bst::load_bst_file(const std::string &bst_filename,
                                 pvstn_t vbinary_stn,
                                 binary_file_meta_t &bst_meta) {
  try {
    auto bst_file = open_bst_for_read(bst_filename);
...
void dna_io_bst::write_bst_file(const std::string &bst_filename,
                                pvstn_t vbinary_stn,
                                binary_file_meta_t &bst_meta) {
  try {
    auto bst_file = open_bst_for_write(bst_filename);
    ...
    std::ostringstream os;
    os << "write_bst_file(): " << e.what();
    throw std::runtime_error(os.str());
  }
}

bool dna_io_bst::write_bst_file(const std::string &bst_filename,
                                vdnaStnPtr *vStations, pvstring vUnusedStns,
