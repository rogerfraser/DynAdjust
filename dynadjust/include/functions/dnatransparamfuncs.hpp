...
void determineGDA2020Parameters(transformation_parameter_set& tParam)
{
    ...
    case NAD83_CSRS_V8_i:
        // No direct parameters exist!
        throw RefTranException(
            message_parameters_undefined<std::string, UINT32>(tParam.from_to_.first, tParam.from_to_.second),
            REFTRAN_DIRECT_PARAMS_UNAVAILABLE);
...
template <typename U>
void determineNAD83Parameters(transformation_parameter_set& tParam)
{
	// Which 'to' datum?
	switch (tParam.from_to_.second)
	{
