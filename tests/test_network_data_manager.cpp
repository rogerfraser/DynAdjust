#define TESTING_MAIN
#define __BINARY_NAME__ "test_network_data_manager"
#define __BINARY_DESC__ "Unit tests for NetworkDataManager class"
...
#include "../dynadjust/dynadjust/dnaadjust/network_data_manager.hpp"
...
TEST_CASE("NetworkDataManager constructor", "[network_data_manager][basic]") {
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
    REQUIRE(true); // Constructor should not throw
}

// Test successful loading with file existence handling
TEST_CASE("Load network files with error handling", "[network_data_manager][load]") {
    resetCallbackFlags();
    ...
    bool result = manager.loadNetworkFiles(&bstBinaryRecords, bst_meta, &vAssocStnList, &bmsBinaryRecords, bms_meta,
...
TEST_CASE("Phased mode configuration", "[network_data_manager][phased]") {
    resetCallbackFlags();
...
TEST_CASE("Station loading failure", "[network_data_manager][error]") {
    resetCallbackFlags();
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
...
TEST_CASE("ASL loading failure", "[network_data_manager][error]") {
    resetCallbackFlags();
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
...
TEST_CASE("Measurement loading failure", "[network_data_manager][error]") {
    resetCallbackFlags();
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
...
TEST_CASE("Callback setters", "[network_data_manager][callbacks]") {
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
...
// Test move semantics
TEST_CASE("Move semantics", "[network_data_manager][move]") {
    auto settings = createTestSettings();
    NetworkDataManager manager1(settings);

    // Test move constructor
    NetworkDataManager manager2 = std::move(manager1);

    REQUIRE(true);
}

// Test copy semantics are deleted
TEST_CASE("Copy semantics deleted", "[network_data_manager][copy]") {
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);

    // These should not compile:
    // NetworkDataManager manager2(manager);     // Copy constructor deleted
    // NetworkDataManager manager3 = manager;    // Copy constructor deleted
...
TEST_CASE("Handle null pointers", "[network_data_manager][null]") {
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
    ...
    bool result = manager.loadNetworkFiles(nullptr, bst_meta, nullptr, nullptr, bms_meta, v_ISL, &v_blockStationsMap,
...
TEST_CASE("Parameter calculation logic", "[network_data_manager][params]") {
    resetCallbackFlags();
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
...
TEST_CASE("Different adjustment modes", "[network_data_manager][modes]") {
...
TEST_CASE("Error handler functionality", "[network_data_manager][error_handler]") {
    resetCallbackFlags();
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
...
TEST_CASE("Measurement count updater", "[network_data_manager][updater]") {
    resetCallbackFlags();
    auto settings = createTestSettings();
    NetworkDataManager manager(settings);
